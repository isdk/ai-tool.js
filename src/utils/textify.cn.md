...
### 3. 单项容器简化 (Single Item Container Simplification)

为了消除冗余的包裹符号并使输出更接近原生 YAML 体验，`textify` 对单项容器（只有一个键的对象或只有一个元素的数组）进行了特殊处理：

* **根节点对象 (Root Object)**: 只有一个键的对象会直接展开。
  * `{ a: 1 }` -> `* a: 1` (不再是 `{a: 1}`)
* **根节点数组 (Root Array)**: 只有一个元素的数组会直接展开。
  * `[ 1 ]` -> `- 1` (不再是 `[1]`)
* **数组中的对象元素 (Object in Array)**: 数组中的单键对象会被简化。
  * `[{ a: 1 }]` -> `- a: 1` (不再是 `- {a: 1}`)
* **原则**: 这种简化仅在不破坏结构的前提下进行。例如，嵌套数组中的单项数组依然会保留容器标识（或采用阶梯式缩进），以确保层级关系清晰且符合“行内化始终只有一行”的原则。

### 4. 字符串引用策略 (String Quoting)
