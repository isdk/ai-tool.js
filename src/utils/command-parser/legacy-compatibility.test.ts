import { parseObjectArguments, parseCommand, ChoiceArgProcessor, TemplateArgProcessor, AIArgProcessor, ObjectArgsToArgsInfo } from './index';

describe('Legacy Compatibility Tests (from original parse-command.test.ts)', () => {
  ...
  describe('parseObjectArguments', async () => {
    test('should parse choices argument', async () => {
      ...
      expect(result).toEqual({choice:{items: ['plant', 'animal']}, 0:"arg2", 1:"arg3"});
    });
    test('should parse choices argument with options', async () => {
      ...
      expect(result).toEqual({choice:{items: ['plant', 'animal'], maxPick: 2}, 0:"arg2", 1:"arg3"});
    ...
    test('should parse named argument mixed with positional arg', async () => {
      const argsStr = 'arg2= False, TRUE';
      const result = await parseObjectArguments(argsStr);
      expect(result).toStrictEqual({0: true, arg2: false});
    });
  ...
  describe('parseCommand function', async () => {
    ...
    it('should handle commands with scope', async () => {
      ...
      expect(await parseCommand(commandStr, scope)).toEqual({ command: 'command', args: {arg1: scope.name, 0: scope.age, age: scope.age, arg2:' "  ' } });
    });

    it('should handle commands with undefined property', async () => {
      const scope = {name: 'John', age: 20}
      const commandStr = 'command(arg1=noSuchValue, age, arg2=" \\"  ")';
      expect(await parseCommand(commandStr, scope)).toEqual({ command: 'command', args: {arg1: undefined, 0: scope.age, age: scope.age, arg2:' "  ' } });
    });
  });
